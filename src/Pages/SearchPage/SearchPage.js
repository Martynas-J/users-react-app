...
const SearchPage = () => {
  ...
  allSearch = Object.entries(data).map((item, index) =>
    item[1].length > 0 ?
      <div key={index} className="search-item">
        ...
        <ul>
          {item[1].map(element => {
            let searchResult = item[0] === "comments" || item[0] === "users" ? element.name : `Title: ${element.title}`
  ...
  return (
    <div id="search">
      {allSearch.every(item => item !== "") ? allSearch : <h1>Nothing Found</h1>}
    </div>
  )
}
